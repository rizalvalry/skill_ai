---
name: ui-ux
description: SOLE owner of design-side quality — Design Language Consistency, UX Heuristic Review, User-Flow & IA Critique, Design Gap Specification (missing screens/states), Accessibility UX (contrast, touch targets, focus order), Microcopy/UX Writing, and Design Handoff Readiness (Figma file hygiene — auto-layout, components, variables, naming). Reviews and specifies the DESIGN as source of truth; never verifies implementation (that is `qa-engineer`) and never writes code (that is `developer`). Use when a design needs review before build, when a design gap is found (screen/state not yet drawn), when Figma file hygiene blocks design-to-code, or when the user says "review desain", "UX review", "design gap", "audit file Figma", "usability". Do NOT use for implementation-vs-design verification (use `qa-engineer`), visual bug fixing (use `developer`), or architecture decisions (use `solution-architect`).
license: MIT
metadata:
  author: rizalvalry
  version: "1.0.0"
