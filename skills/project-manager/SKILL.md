...
| Workspace Analysis Report — as-is technical stack, architecture topology, business domain & processes, workflows, user flows documented from code; health scorecard, hotspots, technical-debt register, prioritized action plan with routes, trend vs previous report | `workspace-analyst` | commission `/analyze` as the baseline before remediation planning or a handover; route each register row to its named owner — never grade, prioritize, or describe the system yourself |
...
| "analisa project", "cek kesehatan repo", "technical debt", "audit codebase", "dokumentasikan sistem ini", "gambarkan bisnis proses / user flow / arsitektur dari kode", inherited/unknown workspace, "apa yang harus dibenahi dulu" | `workspace-analyst` | `workspace-analyst` (inherit) — `/analyze` (report saved by the main session) |
...
| `workspace-analyst` | Workspace Analysis Report §1–§15: scope & method with commands run, inventory, stack table, topology tables + diagram, processes with rules as coded, workflows, user flows + permission matrix, scorecard with grade + confidence + evidence, hotspots, Technical Debt Register with a route per row, top-5 action plan, Not verified; saved path from the main session | A hedging word in §1–§8; a statement in §4–§8 without `file:line`; a diagram node without a table row; a grade without Observed evidence; overall grade averaged; a rewrite/migration recommended, a root cause named, or business intent stated instead of routed; a vulnerability declared without tool output; a register row with no owner |
